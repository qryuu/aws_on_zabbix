...
yum install zabbix-server-mysql zabbix-web-mysql-scl zabbix-web-japanese zabbix-apache-conf-scl zabbix-java-gateway zabbix-agent zabbix-get zabbix-sender zabbix-js httpd mariadb-server -y
elif [ `echo "${version} >= 5.0" | bc` == 1 ] && [ ${minorversion} != "latest" ] && [ ${amazonlinux} = "amzn2" ]; then
amazon-linux-extras install lamp-mariadb10.2-php7.2 epel php7.2 -y
yum install zabbix-server-mysql-${minorversion} zabbix-web-mysql-scl-${minorversion} zabbix-apache-conf-scl-${minorversion} zabbix-web-japanese-${minorversion} zabbix-java-gateway-${minorversion} zabbix-agent-${minorversion} zabbix-get-${minorversion} zabbix-sender-${minorversion} zabbix-js-${minorversion} httpd mariadb-server -y
