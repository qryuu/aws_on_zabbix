...
echo [serverworks-zabbix] >> /etc/yum.repos.d/serverworks.repo
echo name=Serverworks-Zabbix >> /etc/yum.repos.d/serverworks.repo
echo baseurl=https://s3-ap-northeast-1.amazonaws.com/serverworks-yum-repos/zabbix/$amazonlinux/$version/\$basearch >> /etc/yum.repos.d/serverworks.repo
echo gpgcheck=0 >> /etc/yum.repos.d/serverworks.repo
fi
#パッケージインストール
yum update
...
sed -i -e "s/# TLSPSKFile=/TLSPSKFile=\/etc\/zabbix\/tls\/.zabbix_agentd.psk/g" /etc/zabbix/zabbix_proxy.conf
